...
import { LeaveRequest, LeaveBalance, dataService } from '../../../services/dataService';
...
const AdvancedExportPage: React.FC<AdvancedExportPageProps> = ({ onBack }) => {
  ...
  const [previewData, setPreviewData] = useState<LeaveRequest[]>([]);
  ...
  const leaveBalances = dataService.getLeaveBalances();
  const employees = [...new Set(leaveRequests.map(r => ({ id: r.employeeId, name: r.employeeName })))];
  ...
  const getFilteredData = () => {
    return leaveRequests.filter(request => {
      // Date range filter
      if (filters.dateRange.start && new Date(request.appliedDate) < new Date(filters.dateRange.start)) {
        return false;
      }
      if (filters.dateRange.end && new Date(request.appliedDate) > new Date(filters.dateRange.end)) {
        return false;
      }

      // Employee filter
      if (filters.employees.length > 0 && !filters.employees.includes(request.employeeId)) {
        return false;
      }

      // Department filter (would need to be added to LeaveRequest interface in real app)
      if (filters.departments.length > 0) {
        // For demo, we'll skip this filter since department isn't in LeaveRequest
      }

      // Leave type filter
      if (filters.leaveTypes.length > 0 && !filters.leaveTypes.includes(request.leaveType)) {
        return false;
      }
      ...
      // Days range filter
      if (filters.minDays !== null && request.days < filters.minDays) {
        return false;
      }
      if (filters.maxDays !== null && request.days > filters.maxDays) {
        return false;
      }
  ...
  const exportToCSV = async (data: LeaveRequest[]) => {
    const headers = [
      'Employee Name',
      'Employee ID',
      'Leave Type',
    ...
    if (filters.includeBalances) {
      headers.push('Vacation Balance', 'Sick Balance', 'Personal Balance');
    }
    ...
    const csvContent = [
      ...
      ...data.map(request => {
        ...
        if (filters.includeApprovers) {
          row.push(request.approvedBy ? `"${request.approvedBy}"` : '');
        }
        ...
        if (filters.includeBalances) {
          ...
          if (balance) {
            row.push(
              ...
              `${balance.personal.total - balance.personal.used}`
            );
          } else {
    ...
    a.href = url;
    a.download = `leave-export-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const exportToExcel = async (data: LeaveRequest[]) => {
  ...
  const exportToPDF = async (data: LeaveRequest[]) => {
  ...
  const handleArrayFilterChange = (key: keyof Pick<ExportFilters, 'employees' | 'departments' | 'leaveTypes' | 'statuses'>, value: string) => {
    setFilters(prev => ({
      ...prev,
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Advanced Leave Export</h1>
              <p className="text-gray-600 mt-1">Configure detailed filters and export leave management data</p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Leave Types ({filters.leaveTypes.length} selected)
                    </label>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {leaveTypes.map(type => (
                        <label key={type} className="flex items-center">
                          <input
                            type="checkbox"
                            checked={filters.leaveTypes.includes(type)}
                            onChange={() => handleArrayFilterChange('leaveTypes', type)}
                            className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                          />
                    ...
                    {filters.leaveTypes.length === 0 && (
                      <p className="text-xs text-gray-500 mt-1">All leave types will be included</p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Request Status ({filters.statuses.length} selected)
                    </label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <input
                          ...
                          onChange={(e) => handleFilterChange('minDays', e.target.value ? parseInt(e.target.value) : null)}
                      ...
                      <div>
                        <input
                          ...
                          onChange={(e) => handleFilterChange('maxDays', e.target.value ? parseInt(e.target.value) : null)}
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-sm text-gray-700">Include Comments & Notes</span>
                  </label>
                  
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={filters.includeBalances}
                      onChange={(e) => handleFilterChange('includeBalances', e.target.checked)}
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-sm text-gray-700">Include Current Leave Balances</span>
                  </label>
                </div>
              </div>
          ...
          <div className="xl:col-span-1 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{filteredData.filter(r => r.status === 'pending').length}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Approved Requests:</span>
                    <span className="font-medium">{filteredData.filter(r => r.status === 'approved').length}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Rejected Requests:</span>
                    <span className="font-medium">{filteredData.filter(r => r.status === 'rejected').length}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Total Leave Days:</span>
                    <span className="font-medium">{filteredData.reduce((sum, r) => sum + r.days, 0)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Leave Types:</span>
                  <span className="font-medium">
                    {filters.leaveTypes.length === 0 ? 'All' : `${filters.leaveTypes.length} selected`}
                  </span>
                </div>
                ...
                {(filters.minDays !== null || filters.maxDays !== null) && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Duration:</span>
                    <span className="font-medium">
                      {filters.minDays !== null && filters.maxDays !== null
                        ? `${filters.minDays}-${filters.maxDays} days`
                        : filters.minDays !== null
                        ? `≥${filters.minDays} days`
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Include Approvers:</span>
                  <span className="font-medium">{filters.includeApprovers ? 'Yes' : 'No'}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Include Comments:</span>
                  <span className="font-medium">{filters.includeComments ? 'Yes' : 'No'}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Include Balances:</span>
                  <span className="font-medium">{filters.includeBalances ? 'Yes' : 'No'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600">Preview of {filteredData.length} records to be exported</p>
              </div>
              <button
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredData.slice(0, 50).map(request => (
                    <tr key={request.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2">{request.days} days</td>
                      ...
                      <td className="px-4 py-2">{new Date(request.appliedDate).toLocaleDateString()}</td>
                      {filters.includeApprovers && (
                        <td className="px-4 py-2">{request.approvedBy || '-'}</td>
                      )}
                    </tr>
                  ))}
                </tbody>
